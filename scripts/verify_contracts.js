...
async function main() {
	...
	if(conf.LandERC721) {
		console.log("Submitting LandERC721 source code for verification at %o", conf.LandERC721);
		verification_tasks.push(hre.run("verify:verify", {
			address: conf.LandERC721,
		}));
	}
	if(conf.LandSale) {
		console.log("Submitting LandSale source code for verification at %o", conf.LandSale);
		verification_tasks.push(hre.run("verify:verify", {
			address: conf.LandSale,
			constructorArguments: [conf.LandERC721, 0, 4_000_000_000, 1_000, 1]
		}));
	}
